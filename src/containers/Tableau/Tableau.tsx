...
export default function Tableau() {
  ...
  // ER: En fait, ce n'est pas ce qui était demandé. Il fallait afficher les cartes toutes retournées avec un message disant que l'utilisateur a perdu.
  useEffect(() => {
    if (coupsRestant <= 0) {
      redemarrer();
    }
  ...
  return (
    <tableauContexte.Provider value={{ handleSelection, paire }}>
      <main className={styles['tableau-container']}>
        <h3>Nombre de coups restant: {coupsRestant}</h3>
